...
namespace Sap.Handler.Service
{
    public class AfterSaleReturnHandler : ICapSubscribe
    {
        ...
        public async Task HandleReceiptCreditVouchers(AddOrUpdateQuotationReq obj)
        {
            var errorMsg = "";
            var eCode = 0;
            var eMesg = "";
            var docNum = "";
            var quotation = await UnitWork.Find<Quotation>(q => q.SalesOrderId==obj.SalesOrderId).Include(q => q.QuotationMergeMaterials).FirstOrDefaultAsync();
        ...
        public async Task HandleReceiptCreditVouchersERP(string docNum,int? InvoiceDocEntry)
        {
            ...
            {
                //应收贷项凭证
                var ORINmodel = await UnitWork.Find<ORIN>(o => o.DocEntry == int.Parse(docNum)).AsNoTracking().FirstOrDefaultAsync();
                var RIN1model = await UnitWork.Find<RIN1>(o => o.DocEntry == int.Parse(docNum)).AsNoTracking().ToListAsync();
                ...
                await UnitWork.AddAsync<sale_orin, int>(sale_Orin);
                await UnitWork.BatchAddAsync<sale_rin1, int>(sale_Rin1.ToArray());
                //仓库数量
                List<string> itemcodes = sale_Rin1.Select(s => s.ItemCode).ToList();
                List<string> WhsCodes = sale_Rin1.Select(s => s.WhsCode).ToList();
                var oitwList = await UnitWork.Find<OITW>(o => itemcodes.Contains(o.ItemCode) && WhsCodes.Contains(o.WhsCode)).Select(o => new { o.ItemCode, o.IsCommited, o.OnHand, o.OnOrder }).AsNoTracking().ToListAsync();
                ...
                var oitmList = await UnitWork.Find<OITM>(o => itemcodes.Contains(o.ItemCode)).Select(o => new { o.ItemCode, o.IsCommited, o.OnHand, o.OnOrder, o.LastPurCur, o.LastPurPrc, o.LastPurDat, o.UpdateDate }).AsNoTracking().ToListAsync();
                ...
                var oinvCount = await UnitWork.Find<sale_oinv>(s => s.sbo_id == Define.SBO_ID && s.DocEntry == sale_Oinv.DocEntry).CountAsync();
                var inv1Count = await UnitWork.Find<sale_inv1>(s => s.sbo_id == Define.SBO_ID && docenrtys.Contains(s.DocEntry)&& lineNums.Contains(s.LineNum)).CountAsync();
                if (oinvCount > 0)
                {
                    await UnitWork.UpdateAsync<sale_oinv>(sale_Oinv);
                }
                else 
                {
                    await UnitWork.AddAsync<sale_oinv, int>(sale_Oinv);
                }
                if (inv1Count > 0)
                {
                    await UnitWork.BatchUpdateAsync<sale_inv1>(sale_Inv1.ToArray());
                }
                else
                {
                    await UnitWork.BatchAddAsync<sale_inv1, int>(sale_Inv1.ToArray());
                }
