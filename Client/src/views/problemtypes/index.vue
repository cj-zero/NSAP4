  ...
  <sticky :className="'sub-navbar'">
    ...
    <el-table  ref="mainTable"  :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      style="width: 100%;" @row-click="rowClick"  @selection-change="handleSelectionChange">
       ...
       <el-table-column type="selection" align="center"  width="55"></el-table-column>
               ...
               <el-table-column prop="inuseFlag" label="InuseFlag" show-overflow-tooltip></el-table-column>
               <el-table-column prop="parentId" label="ParentId" show-overflow-tooltip></el-table-column>
            <el-table-column min-width="80px" label="OrderIdx">
            <template slot-scope="scope">
              <span>{{scope.row.orderIdx|filterInt}}</span>
            </template>
          </el-table-column>
               <el-table-column prop="prblmTypID" label="prblmTypID" show-overflow-tooltip></el-table-column>
            <el-table-column min-width="80px" label="parentTypeID">
            <template slot-scope="scope">
              <span>{{scope.row.parentTypeID|filterInt}}</span>
            </template>
          </el-table-column>
    ...
    <el-dialog v-el-drag-dialog   class="dialog-mini" width="500px" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="100px">
              ...
              <el-form-item size="small" label="Name" prop="name">
                <el-input v-model="temp.name"></el-input>
              </el-form-item>
              <el-form-item size="small" label="Description" prop="description">
                <el-input v-model="temp.description"></el-input>
              </el-form-item>
              <el-form-item size="small" label="InuseFlag" prop="inuseFlag">
                <el-input v-model="temp.inuseFlag"></el-input>
              </el-form-item>
              <el-form-item size="small" label="ParentId" prop="parentId">
                <el-input v-model="temp.parentId"></el-input>
              </el-form-item>
            <el-form-item size="small" label="OrderIdx">
              <el-select class="filter-item" v-model="temp.orderIdx" placeholder="Please select">
                 <el-option v-for="item in  statusOptions" :key="item.key" :label="item.display_name" :value="item.key">
                </el-option>
              </el-select>
            </el-form-item>
              <el-form-item size="small" label="prblmTypID" prop="prblmTypID">
                <el-input v-model="temp.prblmTypID"></el-input>
              </el-form-item>
            <el-form-item size="small" label="parentTypeID">
              <el-select class="filter-item" v-model="temp.parentTypeID" placeholder="Please select">
                 <el-option v-for="item in  statusOptions" :key="item.key" :label="item.display_name" :value="item.key">
                </el-option>
              </el-select>
            </el-form-item>
