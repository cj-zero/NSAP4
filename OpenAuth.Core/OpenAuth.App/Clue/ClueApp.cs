...
namespace OpenAuth.App
{
    ...
    public class ClueApp : OnlyUnitWorkBaeApp
    {
        ...
        private ILogger<ClueApp> _logger;
        public ClueApp(IHttpClientFactory _httpClient, ILogger<ClueApp> logger, ServiceBaseApp serviceBaseApp, IUnitWork unitWork, IAuth auth) : base(unitWork, auth)
        {
        ...
        public async Task<List<CluePattern>> GetCluePattern(string pattern)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("select * from clueclientutility u where (u.UserTag = \"{0}\" ||  u.UserTag= \"{1}\") AND LOCATE(\"{2}\" ,u.name)  > 0 ", slpCode, userId, pattern);
        ...
        {
            _logger.LogInformation("运行修改线索状态定时任务");
            var result = true;
            ...
            if (updatedClueJob.Count != 0)
            {
                ...
                _logger.LogInformation("运行修改线索状态定时任务，Job修改线索参数为"+ JsonConvert.SerializeObject(logList));
                var clueList = UnitWork.Find<Repository.Domain.Serve.Clue>(a=> updateClueIDs.Contains(a.Id) && a.Status!=0 ).ToList();
                _logger.LogInformation("运行修改线索状态定时任务，实际修改线索参数为" + JsonConvert.SerializeObject(clueList));
