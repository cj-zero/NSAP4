	...
	<div style="height: 100%;">
		<el-container class="container">
			<el-aside
				width="300px"
				theme="light"
				class="select-area" v-if="!isShowContent">
				<el-row>
            ...
            <el-button-group>
              ...
              <template v-for="(commonNode, index) in field.commonNodes">
				...
				<el-row>
					...
					<div align="center">
            <draggable tag="el-row" :list="field.laneNodes"
							...
							:move="handleMove"
						>
							<el-col :span="12" v-for="(laneNode, index) in field.laneNodes" :key="index">
								<div class="node-item" :type="laneNode.type" belongto="laneNodes">
									<el-icon style="font-size: 16px;" :class="laneNode.icon" /> {{ laneNode.name }}
								</div>
							</el-col>
						</draggable>
					</div>
    ...
    groupSchemeContent() {
      ...
      if (!obj.attr || !obj.attr.id) {
        ...
        nodes.length > 0 && nodes.forEach(item => {
          item.top = item.top < 3000 ? 3000 + Number(item.top) : item.top
          item.left = item.left < 3000 ? 3000 + Number(item.left) : item.left
...
//确定当下使用的工具方式
    ...
    showLinkContextMenu(e) {
      ...
      event.preventDefault()
      event.stopPropagation()
      // document.getElementsByClassName('vue-contextmenuName-flow-menu')[0].display = 'none'
      // document.getElementsByClassName('vue-contextmenuName-node-menu')[0].display = 'none'
