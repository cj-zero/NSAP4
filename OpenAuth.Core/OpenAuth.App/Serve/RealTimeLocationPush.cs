...
namespace OpenAuth.App.Serve
{
    public class RealTimeLocationPush : OnlyUnitWorkBaeApp
    {
        ...
        public async Task PushDataMessage()
        {
            ...
            //未推送过的人员Id
            var noPushId = userList.Except(oldList);

            var noPushUserInfo = await UnitWork.Find<User>(c => noPushId.Contains(c.Id)).Select(c => c.Name).ToArrayAsync();

            var users = await _userManagerApp.LoadByRoleName(new string[] { "呼叫中心", "总经理" });
            var message = $"技术员:{string.Join(",", noPushUserInfo)}已离线一小时，请及时处理。";
            ...
            await _hubContext.Clients.Groups(new List<string>() { "呼叫中心", "总经理" }).SendAsync("SmartScreenInfo", "系统", da1);//大屏数据
        }
    }
