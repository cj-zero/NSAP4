    ...
    public class AppServiceOrderLogApp : BaseApp<AppServiceOrderLog>
    {
        ...
        public async Task<Dictionary<string, object>> GetAppOrderLogList(GetAppOrderLogListReq request)
        {
            ...
            var query = from a in UnitWork.Find<AppServiceOrderLog>(null)
                        ...
                        select new { a, b, c };
            query = query.Where(q => q.b.U_SAP_ID == request.SapOrderId && string.IsNullOrEmpty(q.c.MaterialCode) ? "其他设备" == q.c.ManufacturerSerialNumber : q.c.MaterialCode.Substring(0, q.c.MaterialCode.IndexOf("-")) == request.MaterialType && q.a.LogType == request.LogType);
