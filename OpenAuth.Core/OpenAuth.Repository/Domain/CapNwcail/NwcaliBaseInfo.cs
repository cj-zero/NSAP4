//------------------------------------------------------------------------------
// <autogenerated>
//     This code was generated by a CodeSmith Template.
//
//     DO NOT MODIFY contents of this file. Changes to this
//     file will be lost if the code is regenerated.
//     Author:Yubao Li
// </autogenerated>
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations.Schema;
using System.Text;
using OpenAuth.Repository.Core;

namespace OpenAuth.Repository.Domain.CapNwcail
{
    /// <summary>
	/// 
	/// </summary>
    [Table("nwcalibaseinfo")]
    public partial class NwcaliBaseInfo : Entity
    {
        public NwcaliBaseInfo()
        {
          this.FileVersion= string.Empty;
          this.TesterMake= string.Empty;
          this.TesterModel= string.Empty;
          this.TesterSn= string.Empty;
          this.AssetNo= string.Empty;
          this.SiteCode= string.Empty;
          this.Temperature= string.Empty;
          this.RelativeHumidity= string.Empty;
          this.CertificateNumber= string.Empty;
          this.CallbrationEntity= string.Empty;
          this.Operator= string.Empty;
          this.Comment= string.Empty;
          this.CalibrationType= string.Empty;
          this.TUR= string.Empty;
          this.AcceptedTolerance= string.Empty;
          this.TestInterval= string.Empty;
          this.CreateTime= DateTime.Now;
          this.CreateUserId= string.Empty;
          this.CreateUser= string.Empty;
          this.UpdateTime= DateTime.Now;
          this.UpdateUserId= string.Empty;
          this.UpdateUser= string.Empty;
        }

        
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public DateTime? Time { get; set; }
        /// <summary>
        /// 源文件路径
        /// </summary>
        public string CertPath { get; set; }
        /// <summary>
        /// PDF路径
        /// </summary>
        public string PdfPath { get; set; }
        /// <summary>
        /// CNAS证书PDF路径
        /// </summary>
        public string CNASPdfPath { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string FileVersion { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string TesterMake { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string TesterModel { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string TesterSn { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string AssetNo { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string SiteCode { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string Temperature { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string RelativeHumidity { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public double RatedAccuracyC { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public double RatedAccuracyV { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public int AmmeterBits { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public int VoltmeterBits { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string CertificateNumber { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string CallbrationEntity { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string Operator { get; set; }
        public string OperatorId { get; set; }
        public string TechnicalManager { get; set; }
        public string TechnicalManagerId { get; set; }
        public string ApprovalDirector { get; set; }
        public string ApprovalDirectorId { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string Comment { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string CalibrationType { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public int? RepetitiveMeasurementsCount { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string TUR { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string AcceptedTolerance { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public double K { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string TestInterval { get; set; }
        public virtual List<OpenAuth.Repository.Domain.CapNwcail.NwcaliPlcData> NwcaliPlcDatas { get; set; } = new List<NwcaliPlcData>();
        public virtual List<OpenAuth.Repository.Domain.CapNwcail.NwcaliTur> NwcaliTurs { get; set; } = new List<NwcaliTur>();
        public virtual List<OpenAuth.Repository.Domain.CapNwcail.Etalon> Etalons { get; set; } = new List<Etalon>();
        public virtual List<OpenAuth.Repository.Domain.CapNwcail.PcPlc> PcPlcs { get; set; } = new List<PcPlc>();

        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public System.DateTime CreateTime { get; set; }
        public System.DateTime? ExpirationDate { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        [Browsable(false)]
        public string CreateUserId { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string CreateUser { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public System.DateTime? UpdateTime { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        [Browsable(false)]
        public string UpdateUserId { get; set; }
        /// <summary>
        /// 
        /// </summary>
        [Description("")]
        public string UpdateUser { get; set; }
        public string FlowInstanceId { get; set; }
        /// <summary>
        /// 出证人
        /// </summary>
        [Description("")]
        [Browsable(false)]
        public string Issuer { get; set; }
        /// <summary>
        /// 出证人Id
        /// </summary>
        [Description("")]
        public string IssuerId { get; set; }
        /// <summary>
        /// 校准开始时间
        /// </summary>
        public DateTime? StartTime { get; set; }
        /// <summary>
        /// 校准状态
        /// </summary>
        public string CalibrationStatus { get; set; }
        /// <summary>
        /// 校准方式
        /// </summary>
        public string CalibrationMode { get; set; }
        /// <summary>
        /// 校准工具资产编码
        /// </summary>
        public string ToolAssetCode { get; set; }
        /// <summary>
        /// 相差总秒数        
        /// </summary>
        public string TotalSeconds { get; set; }
    }
}