...
namespace OpenAuth.App
{
    public class CertinfoApp : BaseApp<Certinfo>
    {
        ...
        {
            var res = await UnitWork.Find<NwcaliBaseInfo>(c => !string.IsNullOrWhiteSpace(c.ApprovalDirectorId) && string.IsNullOrWhiteSpace(c.CNASPdfPath) && !c.TesterModel.Contains("V0")).OrderByDescending(c => c.Time).Take(3000).ToListAsync();
        ...
        public async Task<TableData> ProductionOrderList(QueryCertinfoListReq request)
        {
            ...
            var query = from a in UnitWork.Find<product_owor>(c => (c.ItemCode.StartsWith("C") || c.ItemCode.StartsWith("BT") || c.ItemCode.StartsWith("BTE") || c.ItemCode.StartsWith("BE")) && c.CreateDate >= DateTime.Parse("2021-01-01"))
