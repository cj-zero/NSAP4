	...
	public class OrderDraftController : Controller {
		...
		public Response<List<OrderFile>> GetFilesList(string OrderId, string TypeId, string fileSboId) {
			...
			if (!string.IsNullOrEmpty(TypeId)) {
				...
				var sboid = _serviceBaseApp.GetUserNaspSboID(userId);
				int billSboId = 0;
				if (!string.IsNullOrEmpty(fileSboId)) { billSboId = int.Parse(fileSboId); } else { billSboId = sboid; }
		...
		public async Task<TableData> GridRelationContractList(int pageSize, int pageIndex, string filterQuery, string sortname, string sortorder, string itemCode, string cardCode) {
			var result = new TableData();
		...
		public async Task<TableData> CopyProductToSaleSelect(int page, int rp, string qtype, string query, string sortname, string sortorder, int sboID) {
			var result = new TableData();
