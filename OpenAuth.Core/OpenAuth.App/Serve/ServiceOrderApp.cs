            ...
            switch (role)
            {
                ...
                case 6:
                    ...
                    .Take(page_size).ToListAsync()).Select(s => new
                    {
                        ...
                        orderStatus = s.MaterialInfo.Where(c => c.ServiceOrderId == s.Id).All(c => c.OrderTakeType == 0) == true ? 1 : (s.MaterialInfo.Where(c => c.ServiceOrderId == s.Id).All(c => c.Status >= 7) == true ? 2 : 3)
                    }).ToList<object>();
