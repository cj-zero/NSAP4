  ...
  <div v-if="show">
    <el-form label-position="top">
      ...
      <el-form-item :label="'大小'" v-if="Object.keys(data.options).indexOf('size')>=0">
        宽度 <el-input size="small" style="width: 90px;" type="number" v-model.number="data.options.size.width"></el-input>
            ...
            <el-checkbox-group v-model="data.options.defaultValue" size="small">
              ...
              >
                <li v-for="(item, index) in data.options.options" :key="index" >
                  ...
                  <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;"><i class="el-icon-rank" title="拖动"></i></i>
                  <el-button @click="handleOptionsRemove(index)" circle plain type="danger" title="删除" size="mini" icon="el-icon-delete" style="padding: 4px;margin-left: 5px;"></el-button>
      ...
      <el-form-item :label="'默认值'" v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'textarea' || data.type == 'input' ||  data.type == 'text' || data.type=='rate' || data.type=='color' || data.type=='switch')">
        ...
        <el-color-picker 
          ...
          :show-alpha="data.options.showAlpha" size="small"
        ></el-color-picker>
      ...
      <template v-if="data.type == 'time' || data.type == 'date'">
        ...
        <el-form-item :label="'是否为范围选择'" v-if="data.type == 'time'">
          <el-switch
            v-model="data.options.isRange" size="small"
          >
          </el-switch>
        </el-form-item>
        <el-form-item :label="'是否获取时间戳'" v-if="data.type == 'date'">
          <el-switch
            v-model="data.options.timestamp" size="small"
          >
          </el-switch>
        </el-form-item>
        ...
        <el-form-item :label="'开始时间'" v-if="(data.options.isRange) || data.options.type=='datetimerange' || data.options.type=='daterange'">
          <el-input size="small" v-model="data.options.startPlaceholder"></el-input>
        </el-form-item>
        <el-form-item :label="'结束时间'" v-if="data.options.isRange || data.options.type=='datetimerange' || data.options.type=='daterange'">
        ...
        <el-form-item :label="'默认值'" v-if="data.type=='time' && Object.keys(data.options).indexOf('isRange')>=0">
          <el-time-picker 
            ...
            :value-format="data.options.format" size="small"
          >
          </el-time-picker>
          <el-time-picker 
            key="2"
        ...
        <el-form-item :label="'列配置项'">
          ...
          >
            <li v-for="(item, index) in data.columns" :key="index" >
              ...
              <el-button @click="handleOptionsRemove(index)" circle plain type="danger" size="mini" icon="el-icon-delete" title="删除" style="padding: 4px;margin-left: 5px;"></el-button>
      ...
      <template v-if="data.type != 'grid' && data.type !== 'text'">
        <el-form-item :label="'操作属性'">
          <el-checkbox size="small" v-model="data.options.readonly" v-if="Object.keys(data.options).indexOf('readonly')>=0">只读</el-checkbox>
          <el-checkbox size="small" v-model="data.options.disabled" v-if="Object.keys(data.options).indexOf('disabled')>=0">禁用</el-checkbox>
          <el-checkbox size="small" v-model="data.options.editable" v-if="Object.keys(data.options).indexOf('editable')>=0">文本框可输</el-checkbox>
...
export default {
  ...
  methods: {
    ...
    validateRequired(val) {
      if (val) {
        this.validator.required = { required: true, message: `${this.data.model}必须填写` }
      } else {
    ...
    validateDataType(val) {
      ...
      if (val) {
        this.validator.type = { type: val, message: `${this.data.model}格式不正确` }
      } else {
    ...
    valiatePattern(val) {
      ...
      if (val) {
        this.validator.pattern = { pattern: val, message: `${this.data.model}格式不匹配` }
      } else {
