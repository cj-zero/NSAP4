...
namespace OpenAuth.WebApi.Controllers.Order
{
    ...
    public class BillMaterialsController : Controller
    {
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("GetCKCountsByCK")]
        public TableData GetCKCountsByCK(string ItemCode, string WhsCode, string SboId)
        {
            var result = new TableData();
            try
            {
                result.Data = _serviceSaleOrderApp.GetCKCountsByCK(ItemCode, WhsCode, SboId);
            }
            catch (Exception e)
            {
                result.Message = e.Message;
